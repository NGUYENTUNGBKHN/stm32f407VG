...
int boot_main()
{
    ...
    if (1)
    {
        TRACE_INFO("jump to application\n");
        jump_to_application();
    }
    else
    {
        TRACE_INFO("jump to bootloader\n");
        jump_to_bootloader();
    }
    while (1)
    {
...
static void jump_to_application()
{
    ...
    uint32_t app_msp_adr = *(volatile uint32_t *)(APP_START_ADDRESS);
    uint32_t app_reset_handler_adr = *(volatile uint32_t *)(APP_START_ADDRESS + 4);

    __set_MSP(app_msp_adr);

    reset_handler = (void *)app_reset_handler_adr;
